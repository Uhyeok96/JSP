...
public class BoardDAO extends JDBConnect{
	...
	public BoardDTO selectView(String num) {
		// 메서드 호출시 입력은 num(board pk) 받고 가져온 데이터를 BoardDTO객체에 넣어 리턴
		BoardDTO viewDTO = new BoardDTO();
		
		// 3단계 (쿼리문 생성) member pk -> board fk
		//String query = "select * from board where num=?"; // 작성자임??? 판단불가
		
		// member에 있는 작성자를 가져올 수 있도록 join 처리용
		
		String query = "select B.*, M.name from member M inner join board B on M.id= B.id where num=? ";
		// member 테이블의 별칭은 M 으로 board 테이블의 별칭은 B로 선언
		// 부모 테이블은 M에 이너 조인 B를 이용하고 id가 같은 자료를 찾음 M.id= B.id
